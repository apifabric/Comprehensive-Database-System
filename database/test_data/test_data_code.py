...
while restart_count < 5 and has_errors:
    ...
    try:
        if not -6718606698600397794 in succeeded_hashes:  # avoid duplicate inserts
            instance = user1 = User(name="John Doe", email="john.doe@example.com", created_at=datetime.datetime(2021, 1, 1), profile_id=1)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(-6718606698600397794)
    except Exception as e:
        has_errors = True
    ...
    try:
        if not 147431823876555841 in succeeded_hashes:  # avoid duplicate inserts
            instance = user2 = User(name="Jane Smith", email="jane.smith@example.com", created_at=datetime.datetime(2021, 2, 15), profile_id=2)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(147431823876555841)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not -598391386242412545 in succeeded_hashes:  # avoid duplicate inserts
            instance = user3 = User(name="Alice Jones", email="alice.jones@example.com", created_at=datetime.datetime(2021, 3, 30), profile_id=3)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(-598391386242412545)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not 2359168575720029308 in succeeded_hashes:  # avoid duplicate inserts
            instance = user4 = User(name="Bob Brown", email="bob.brown@example.com", created_at=datetime.datetime(2021, 4, 20), profile_id=4)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(2359168575720029308)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
